...
import { getFriendsSend } from "./FriendSlice";
...
function FriendSend() {
  ...
  const FriendsSend = currentPageUsers.map((id) => usersById[id]);
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(getFriendsSend({ filterName, page }));
  ...
  return (
    <Container>
      <Typography variant="h4">Friend Send</Typography>
      <Card sx={{ p: 3 }}>
        <Stack spacing={2}>
        ...
        <FriendsCard friends={FriendsSend} />
      </Card>
    </Container>
  );
}
